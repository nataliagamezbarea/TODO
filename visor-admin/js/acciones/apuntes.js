  ...
  if (!nuevoVal && it.inc_apunte) {
    ...
    try {
      const res = await fetch('/api/eliminar_apunte', {
        ...
        body: JSON.stringify({ grado: grad, archivo: it.archivo, nombre_apunte: nombre, borrar_pdfs: true })
      });
...
async function confirmGenerateApunteLatex() {
  ...
  try {
    const res = await fetch('/api/compilar_apunte', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ grado: grad, archivo: it.archivo, nombre_apunte: nombre })
    });
    const data = await res.json();
    hideBlocker();
    if (!data.ok) {
