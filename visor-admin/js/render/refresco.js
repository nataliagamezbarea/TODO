...
async function refrescarPaginaPDFJS(containerId, pno) {
  ...
  try {
    ...
    const urlPDF = `/api/preview/${grad}?${st.qs}&t=${Date.now()}`;
    ...
    try {
      ...
      const resI = await fetch(`/api/doc_info/${grad}?${qsDoc}`);
      const info = await resI.json();
      const meta = (info && info.pages && info.pages[pno]) ? info.pages[pno] : { page_num: pno };
      let metaOrig = null;
      if (!isLeft) {
        const resO = await fetch(`/api/doc_info/${grad}?archivo=${encodeURIComponent(it.archivo)}&mode=old&t=${Date.now()}`);
