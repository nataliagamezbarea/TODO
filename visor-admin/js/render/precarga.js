...
function _ejecutarPrecargaProximos() {
  ...
  candidatos.forEach(idx => {
    ...
    _obtenerBufferPDF(`/api/preview/${grad}?${qsOld}`).then(() => {
      _obtenerDocInfoJSON(`/api/doc_info/${grad}?${qsOld}`).catch(() => {});
      ...
      if (it.decision !== 'original') {
        _obtenerBufferPDF(`/api/preview/${grad}?${qsNew}`).catch(() => {});
        _obtenerDocInfoJSON(`/api/doc_info/${grad}?${qsNew}`).catch(() => {});
      }
    }).catch(() => {});
  });
}
...
function precargarPrimerosOriginales() {
  ...
  primeros.forEach(it => {
    const qsOld = `archivo=${encodeURIComponent(it.archivo)}&mode=old&enc=1&int=0&col=0&net=0`;
    _obtenerBufferPDF(`/api/preview/${grad}?${qsOld}`).catch(() => {});
    _obtenerDocInfoJSON(`/api/doc_info/${grad}?${qsOld}`).catch(() => {});
  });
}
