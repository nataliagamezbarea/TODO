...
function loadDocViewer(containerId, isLeft, enc, int, col, netVal) {
  ...
  if (it && el) loadDocViewerPDFJS(containerId, isLeft, el, it, isLeft ? 'old' : 'new', enc, int, col, netVal);
}
...
async function loadDocViewerPDFJS(containerId, isLeft, el, it, mode, enc, int, col, netVal) {
  ...
  const urlPDF = `/api/preview/${grad}?${qs}`;

  const promesas = [_obtenerBufferPDF(urlPDF), _obtenerDocInfoJSON(`/api/doc_info/${grad}?${qs}`).catch(() => null)];
  if (!isLeft) promesas.push(_obtenerDocInfoJSON(`/api/doc_info/${grad}?${qsOld}`).catch(() => null));
