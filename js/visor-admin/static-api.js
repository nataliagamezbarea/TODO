...
(() => {
  ...
  let cfgPromise = null;
  let cache = { revision: null, rewrites: {}, branches: null, trees: new Map() };
  ...
  async function getRewrites(g) {
    ...
    let x = null;
    if (repo) {
      // Evitar peticiones 404 repetidas cuando una rama aún no tiene historial.
      const masterTree = await tree(repo, 'master');
      const ruta = `almacen/datos/rewrites_${g}.json`;
      if (masterTree.some(p => p.toLowerCase() === ruta.toLowerCase())) {
        x = await getContent(ruta, 'master', repo);
      }
    }
  ...
  async function cargarContextoRama(repo, branch, pathsDisponibles = []) {
    ...
    for (const archivo of ['APUNTES.csv', 'EJERCICIOS_PRACTICAS_PROYECTOS.csv']) {
      try {
        // Los CSV pueden ser comunes del repositorio y no existir en una rama de pruebas.
        // Solo hacemos la petición cuando sabemos por el árbol que el archivo existe.
        let origen = branch;
        ...
        filas.forEach(linea => {
          ...
          // CSV sencillo: además de soportar comas entrecomilladas, basta con Papa si está disponible.
          let vals = [];
          ...
          archivos.forEach(u => {
            ...
            if (base) mapa.set(base, { asignatura: String(vals[ia] || '').trim(), trimestre: String(vals[it] || '').trim() });
          });
        });
      } catch (_) {}
    }
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (Array.isArray(ramas)) {
          return ramas.map(x => String(x?.name || '').trim())
            .filter(r => r && r.toLowerCase() !== 'master' && !/_limpia$/i.test(r));
        }
      }
    } catch (_) {}
  ...
  async function buildData(ramaObjetivo = null) {
    const c = await getConfig();
    const repo = c.repo || c.publicRepo;
    if (!repo) return {};
    const rev = await getRevision();
    const out = {};
    // El modo TODAS debe incluir TODAS las ramas reales del repositorio, no
    // solamente las tres ramas que estaban declaradas antiguamente en GRADOS.
    // Esto también permite que funcionen ramas nuevas sin modificar este JS.
    const ramasDisponibles = ramaObjetivo
      ? [String(ramaObjetivo).trim()]
      : await listarRamasDatos(repo);
    for (const g of ramasDisponibles) {
      ...
      for (const e of rw || []) {
        ...
        entries.push({ idx: entries.length, archivo:a, rel_path:rel, carpeta:rel.includes('/')?rel.split('/')[0]:'raíz', nombre_limpio:cleanName(a), cambia_nombre:cleanName(a)!==a, inc_nombre:!!rr.inc_renombre, inc_interior:rr.inc_interior !== false, inc_colegio:rr.inc_colegio !== false, inc_internet:!!rr.inc_internet, inc_apunte:!!rr.inc_apunte || rel.startsWith('apuntes/'), nombre_apunte:rr.nombre_apunte||'', latex_compilado:!!rr.latex_compilado, asignatura:contextoRama.get(a.toLowerCase())?.asignatura||'', trimestre:contextoRama.get(a.toLowerCase())?.trimestre||'', old:e.start||'', new:e.new||'', start:e.start||'', end:e.end||'', enunciados_count:Array.isArray(e.enunciados)?e.enunciados.length:0, is_cv:false, cambia:!!(e.new || e.enunciados), include:e.include !== false, visto:!!rr.visto, decision:rr.decision||'' });
      }
      for (const p of pdfs) {
        const a = p.split('/').pop();
        ...
        const rr = rev[key(g,a)] || {};
        out[g] = out[g] || {};
        entries.push({ idx:entries.length, archivo:a, rel_path:p, carpeta:p.includes('/')?p.split('/')[0]:'raíz', nombre_limpio:cleanName(a), cambia_nombre:cleanName(a)!==a, inc_nombre:!!rr.inc_renombre, inc_interior:rr.inc_interior !== false, inc_colegio:rr.inc_colegio !== false, inc_internet:!!rr.inc_internet, inc_apunte:!!rr.inc_apunte || p.startsWith('apuntes/'), nombre_apunte:rr.nombre_apunte||'', latex_compilado:!!rr.latex_compilado, asignatura:contextoRama.get(a.toLowerCase())?.asignatura||'', trimestre:contextoRama.get(a.toLowerCase())?.trimestre||'', old:'', new:'', start:'', end:'', enunciados_count:0, is_cv:false, cambia:false, include:rr.include !== false, visto:!!rr.visto, decision:rr.decision||'' });
      }
  ...
  async function pdfResponse(path){
    ...
    const c=await getConfig();
    const repo=c.repo||c.publicRepo;
    if (!repo || !g || !archivo) return new Response('PDF no encontrado',{status:404});

    const ramaOriginal = GRADOS[g]?.rama || g;
    const ramaLimpia = GRADOS[g]?.limpia || `${g}_limpia`;
    // No solicitar nunca una rama *_limpia que no exista. En repositorios donde
    // todavía no se ha generado esa rama, usamos el PDF original como fallback
    // para evitar bucles de 404 y permitir seguir navegando por el visor.
