...
}); if (typeof load === 'function') await load(); hideBlocker(); if (typeof openPos === 'function') { await openPos(POS); } else if (typeof refreshRightIframe === 'function') { refreshRightIframe(); } liberar(); } catch (err) { hideBlocker(); void 0; liberar(); } return; } if (!aceptar) { /* * "Dejar Original" NO toca el PDF ni las modificaciones.
...
}); if (typeof load === 'function') await load(); if (typeof openPos === 'function') await openPos(POS); liberar(); } catch (err) { void 0; liberar(); } return; } /* * Un archivo marcado como Original no se sube accidentalmente.
...
}); nav(1); liberar(); } catch (err) { hideBlocker(); void 0; liberar(); } } /** * Quita SOLO lo que el usuario ha añadido manualmente con "Añadir"/"Crear"
...
}); if (typeof load === 'function') await load(); hideBlocker(); refreshRightIframe(); window.__accionEnCurso = false; } catch (err) { hideBlocker(); void 0; window.__accionEnCurso = false; } } 